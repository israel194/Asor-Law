...
function paymentTermsBlock(d, monthlyRent, leaseMonths) {
    ...
    if (method === "checks") {
        ...
        return `
            <p>במעמד החתימה על הסכם זה ישלם השוכר למשכיר את דמי השכירות בגין החודש הראשון של תקופת השכירות, בסך של ${rent} ₪.</p>
            <p>בנוסף, ובמעמד החתימה, יפקיד השוכר בידי המשכיר ${num} המחאות דחויות, על סך של ${rent} ₪ כל אחת, אשר תאריכי פירעונן ב-${esc(d.payment_day || "—")} לכל חודש קלנדרי, החל מהחודש השני של תקופת השכירות ועד לחודש האחרון של תקופת השכירות.</p>`;
    }
    if (method === "bank_transfer") {
        return `
            <p>במעמד החתימה על הסכם זה ישלם השוכר למשכיר את דמי השכירות בגין החודש הראשון של תקופת השכירות, בסך של ${rent} ₪.</p>
            <p>החל מהחודש השני של תקופת השכירות, ישלם השוכר את דמי השכירות באמצעות הוראת קבע / העברה בנקאית לחשבון הבנק של המשכיר, ב-${esc(d.payment_day || "—")} לכל חודש קלנדרי, בגין אותו החודש.</p>`;
    }
    return `
        <p>אופן תשלום דמי השכירות: ${PAYMENT_LABEL[method] || "כפי שיוסכם בין הצדדים בכתב"}.</p>
        <p>דמי השכירות בגין כל חודש ישולמו ב-${esc(d.payment_day || "—")} לכל חודש קלנדרי, בגין אותו החודש.</p>`;
}
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .clause { margin: 6pt 0; }
        ol.clauses { padding-right: 24pt; margin: 6pt 0; }
        ol.clauses > li { margin: 6pt 0; }
        .signatures {
            margin-top: 36pt;
            display: flex;
...
<h2>5. מיסים והוצאות</h2>
<p class="clause">המיסים וההוצאות החלים על הדירה ישולמו כדלקמן:</p>
<p class="clause"><strong>א.</strong> ארנונה, מים וביוב, גז, חשמל, כבלים ואינטרנט וכל תשלום שוטף אחר החל על מחזיק יועברו על שם השוכר עם תחילת תקופת השכירות וישולמו ע"י השוכר במועדים הקבועים לתשלומם. השוכר יעביר למשכיר עפ"י דרישתו אסמכתאות המעידות על העברת החיובים השוטפים ע"ש השוכר.</p>
<p class="clause"><strong>ב.</strong> דמי וועד בית בגין אחזקה שוטפת של החלק היחסי בבניין ישולמו ע"י השוכר במועדם. כל העלאה, אם תהיה כזו בדמי וועד הבית, תחול על השוכר בלבד.</p>
<p class="clause"><strong>ג.</strong> השוכר יעביר על שמו את כל החיובים האמורים בתוך 30 ימים ממועד חתימת הסכם זה, וימציא למשכיר העתק מאישור העירייה בדבר חילופי המחזיקים בנכס.</p>
<p class="clause"><strong>ד.</strong> עם סיום הסכם זה מכל סיבה שהיא, הצדדים מתחייבים לפעול יחד על מנת להשיב את רישום החשבונות על שם המשכיר.</p>
<p class="clause"><strong>ה.</strong> תשלומים והוצאות החלים כדרך קבע על בעל הנכס, כגון היטל השבחה והיטל סלילה, יחולו וישולמו על ידי המשכיר.</p>

<h2>6. סיום מוקדם ופיצוי מוסכם</h2>
...
<h2>9. תום תקופת השכירות ופינוי המושכר</h2>
...
<p class="clause">המבוא וסעיפים 2, 3, 4, 6, 7, 8, 9 ו-10 הינם סעיפים יסודיים ועיקריים להסכם זה והפרתם תהווה הפרה יסודית של הוראות ההסכם.</p>
